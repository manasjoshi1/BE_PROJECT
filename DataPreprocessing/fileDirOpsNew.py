        ...
        for file in files:
            ...
            if ext == ".jpg":
                ...
                if frameNumber in validFrames.keys():
                    ...
                    #if shutil.copy2(filePath, newFilePath):
...
def save_to_file(labels_dict, phn_list, path, data_type):
    with open(f'{path}/{data_type}_dict.json', 'w') as f:
        json.dump(labels_dict, f)
    with open(f'{path}/{data_type}_phn.txt', 'w') as file:
        for item in phn_list:
            file.write(item+"\n")
...
def speakerToDict(speakerDir):
    ...
    for root, dirs, files in os.walk(speakerDir):
        for file in files:
            ...
            if extension == ".txt":
                ...
                speakerName = names[0]
                videoName = names[1]
                if videoName not in videoNameList:
                    videoNameList.append(speakerName+"_"+videoName)
    ...
    for root, dirs, files in os.walk(speakerDir):
        for file in files:
            ...
            if extension == ".jpg":
                names = name.split("_")
                phoneme = names[3].split('.')[0]
                #phonemeNumberMap = phoneme_set_39
                #labelNumber = phonemeNumberMap[phoneme]
                file_name = names[0]+'_'+names[1]
                data_labels[file] = phoneme
                data_phn_list.append(file_name +'_PHN.txt')

    return data_labels, data_phn_list

def allSpeakersToDict(databaseDir):
    rootDir = databaseDir
    dirList = []
    for dir in directories(rootDir):
        if 'processed' not in dir and 'volunteers' in dir:
            dirList.append(dir)
    for speakerDir in dirList:
        data_labels_dict = OrderedDict({})
        phn_data = []
        data_labels, phn_data= speakerToDict(speakerDir)
        data_labels_dict.update(data_labels)
        phn_data += phn_data
        phn_data = sorted(list(set(phn_data)))
        print(phn_data)
        save_to_file(data_labels_dict, phn_data, speakerDir, data_type='data')
        print(f"Created image-label dict for: {speakerDir}")
    return 0
